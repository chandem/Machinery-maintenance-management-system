...
from app.schemas.inventory import InventoryCreate, InventoryRead, PartCreate, PartRead, PartTransactionCreate, SupplierCreate, SupplierRead
...
def create_supplier(payload: SupplierCreate, db: Session = Depends(get_db)):
    ...
    item = Supplier(**payload.model_dump()); db.add(item); db.commit(); db.refresh(item)
    return item
...
def create_part(payload: PartCreate, db: Session = Depends(get_db)):
    ...
    item = Part(**payload.model_dump()); db.add(item); db.commit(); db.refresh(item)
    return item
...
def create_inventory(payload: InventoryCreate, db: Session = Depends(get_db)):
    ...
    item = Inventory(**payload.model_dump()); db.add(item); db.commit(); db.refresh(item)
    return item
...
def create_transaction(payload: PartTransactionCreate, db: Session = Depends(get_db)):
    ...
    if payload.transaction_type == "out":
        ...
        inventory.quantity_on_hand = payload.quantity
    tx = PartTransaction(**payload.model_dump()); db.add(tx); db.commit(); db.refresh(tx)
    return {"id": tx.id, "part_id": tx.part_id, "quantity_on_hand": inventory.quantity_on_hand}
