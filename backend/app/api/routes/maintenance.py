...
from app.schemas.maintenance import (
    MaintenancePlanCreate, MaintenancePlanRead, MaintenanceScheduleStatusRead,
    MaintenanceServiceComplete, WorkOrderCreate, WorkOrderRead, WorkOrderUpdate,
    WorkOrderCostRead, WorkOrderPartCreate, WorkOrderPartRead, WorkOrderPartReturn,
    WorkOrderLaborCreate, WorkOrderLaborRead,
)
...
def complete_maintenance_service(plan_id: int, payload: MaintenanceServiceComplete, db: Session = Depends(get_db)):
...
def maintenance_plan_status(db: Session = Depends(get_db)):
    ...
    for plan, equipment in plans:
        ...
        else: schedule_status = "scheduled"
...
def get_work_order_cost(work_order_id: int, db: Session = Depends(get_db)):
    order = db.get(WorkOrder, work_order_id)
    if order is None: raise HTTPException(404, "Work order not found")
    parts = db.scalars(select(WorkOrderPart).where(WorkOrderPart.work_order_id == work_order_id)).all(); parts_cost = Decimal("0")
    for item in parts:
        if item.unit_cost is None: continue
...
def add_work_order_part(work_order_id: int, payload: WorkOrderPartCreate, db: Session = Depends(get_db)):
    ...
    if part is None: raise HTTPException(404, "Part not found")
    inventory = db.scalar(select(Inventory).where(Inventory.part_id == payload.part_id))
    if inventory is None or inventory.quantity_on_hand < payload.quantity: raise HTTPException(400, "Insufficient stock")
    if db.scalar(select(WorkOrderPart).where(WorkOrderPart.work_order_id == work_order_id, WorkOrderPart.part_id == payload.part_id)): raise HTTPException(409, "Part already added to this work order")
    unit_cost = payload.unit_cost if payload.unit_cost is not None else part.unit_cost; inventory.quantity_on_hand -= payload.quantity
    item = WorkOrderPart(work_order_id=work_order_id, part_id=payload.part_id, quantity=payload.quantity, unit_cost=unit_cost); db.add(item)
    ...
    if unit_cost is not None: order.actual_cost = (order.actual_cost or 0) + payload.quantity * unit_cost
...
def list_work_order_parts(work_order_id: int, db: Session = Depends(get_db)):
    if db.get(WorkOrder, work_order_id) is None: raise HTTPException(404, "Work order not found")
...
def return_work_order_part(work_order_id: int, work_order_part_id: int, payload: WorkOrderPartReturn, db: Session = Depends(get_db)):
    ...
    if item is None or item.work_order_id != work_order_id: raise HTTPException(404, "Work order part not found")
    reference = _part_reference(order.work_order_number, item.part_id); returned = db.scalar(select(func.coalesce(func.sum(PartTransaction.quantity), 0)).where(PartTransaction.part_id == item.part_id, PartTransaction.transaction_type == "in", PartTransaction.reference == reference)) or 0
    if payload.quantity > item.quantity - returned: raise HTTPException(400, "Return quantity exceeds remaining issued quantity")
    inventory = db.scalar(select(Inventory).where(Inventory.part_id == item.part_id))
    if inventory is None: raise HTTPException(404, "Inventory record not found")
    inventory.quantity_on_hand += payload.quantity; db.add(PartTransaction(part_id=item.part_id, transaction_type="in", quantity=payload.quantity, unit_cost=item.unit_cost, reference=reference, notes=payload.notes or f"Returned from work order {order.work_order_number}"))
    if item.unit_cost is not None: order.actual_cost = max((order.actual_cost or 0) - payload.quantity * item.unit_cost, 0)
    db.commit(); db.refresh(item); return item
...
def add_work_order_labor(work_order_id: int, payload: WorkOrderLaborCreate, db: Session = Depends(get_db)):
    order = db.get(WorkOrder, work_order_id)
    if order is None: raise HTTPException(404, "Work order not found")
    item = WorkOrderLabor(work_order_id=work_order_id, **payload.model_dump()); db.add(item); order.actual_cost = (order.actual_cost or 0) + payload.hours * payload.hourly_rate
    db.commit(); db.refresh(item); return item
